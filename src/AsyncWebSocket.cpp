...
size_t webSocketSendFrame(AsyncClient *client, bool final, uint8_t opcode, bool mask, uint8_t *data, size_t len){
  ...
  if(buf == NULL){
    Serial.printf("could not malloc %u bytes for frame header\n", headLen);
    return 0;
  }
  ...
  if(client->add((const char *)buf, headLen) != headLen){
    Serial.printf("error adding %lu header bytes\n", headLen);
    free(buf);
    return 0;
  }
  free(buf);
  ...
  if(len){
    ...
    if(client->add((const char *)data, len) != len){
      Serial.printf("error adding %lu data bytes\n", len);
      return 0;
    }
  }
  if(!client->send()){
    Serial.printf("error sending frame: %lu\n", headLen+len);
    return 0;
  }
  return len;
}
 ...
 */
 const char * AWSC_PING_PAYLOAD = "STM32AsyncWebServer-PING";
...
void AsyncWebSocketClient::_onData(void *pbuf, size_t plen){
  ...
  while(plen > 0){
    ...
    if((datalen + _pinfo.index) < _pinfo.len){
      ...
      Serial.printf("frame error: len: %u, index: %llu, total: %llu\n", datalen, _pinfo.index, _pinfo.len);
      //what should we do?
      break;
    }
...
IPAddress AsyncWebSocketClient::remoteIP() {
    if(!_client) return IPAddress((uint32_t) 0);
