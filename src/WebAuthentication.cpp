...
bool checkDigestAuthentication(const char * header, const char * method, const char * username, const char * password, const char * realm, bool passwordIsHash, const char * nonce, const char * opaque, const char * uri){
  if(username == NULL || password == NULL || header == NULL || method == NULL){
    Serial.printf("AUTH FAIL: missing requred fields\n");
    return false;
  }
  ...
  if(nextBreak < 0){
    Serial.printf("AUTH FAIL: no variables\n");
    return false;
  }
  ...
  do {
    ...
    if(eqSign < 0){
      Serial.printf("AUTH FAIL: no = sign\n");
      return false;
    }
    ...
    if(varName.equals("username")){
      if(!avLine.equals(username)){
        Serial.printf("AUTH FAIL: username\n");
        return false;
      }
      myUsername = avLine;
    } else if(varName.equals("realm")){
      if(realm != NULL && !avLine.equals(realm)){
        Serial.printf("AUTH FAIL: realm\n");
        return false;
      }
      myRealm = avLine;
    } else if(varName.equals("nonce")){
      if(nonce != NULL && !avLine.equals(nonce)){
        Serial.printf("AUTH FAIL: nonce\n");
        return false;
      }
      myNonce = avLine;
    } else if(varName.equals("opaque")){
      if(opaque != NULL && !avLine.equals(opaque)){
        Serial.printf("AUTH FAIL: opaque\n");
        return false;
      }
    } else if(varName.equals("uri")){
      if(uri != NULL && !avLine.equals(uri)){
        Serial.printf("AUTH FAIL: uri\n");
        return false;
      }
      myUri = avLine;
  ...
  Serial.printf("AUTH FAIL: password\n");
  return false;
}
